...
public enum MonitorType {
    ...
    FLINKER_EXCEPTION_MONITOR(7,"全量任务异常监控", MonitorCat.FLINKER_MONITOR),
    ...
    public static List<MonitorType> getMonitorTypeListByCat(int catKey) {
        if (catKey == 1) {
            ...
            return Lists.newArrayList(FLINKER_EXCEPTION_MONITOR);
        }else {
            return Lists.newArrayList(TASK_DELAY_MONITOR, TASK_EXCEPTION_MONITOR, TASK_STATUS_MONITOR, TASK_STATUS_MISMATCH_MONITOR, TASK_SYNC_STATUS_MONITOR, WORKER_JVM_STATE_MONITOR, WORKER_RUNNING_STATE_MONITOR, FLINKER_EXCEPTION_MONITOR);
        }
    }
}