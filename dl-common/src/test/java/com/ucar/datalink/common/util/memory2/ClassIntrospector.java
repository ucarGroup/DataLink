...
public class ClassIntrospector {
    ...
    private IdentityHashMap<Object, Boolean> m_visited = new IdentityHashMap<Object, Boolean>( 80000 );
    ...
    private ObjectInfo introspect( final Object obj, final Field fld ) throws IllegalAccessException
    {
        ...
        if ( fld == null )
        {
            root = new ObjectInfo( "", type.getCanonicalName(), 0, getShallowSize( type ),
                    arraySize, baseOffset, indexScale );
        }
        else
        {
            final int offset = ( int ) unsafe.objectFieldOffset( fld );
            root = new ObjectInfo( fld.getName(), type.getCanonicalName(), offset,
