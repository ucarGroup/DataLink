 ...
 */
public class ZhangDa_polestar extends AbstractTriggerSqlInterceptor {

    private static final Logger logger = LoggerFactory.getLogger(ZhangDa_polestar.class);
        ...
        if(record.getEventType()==EventType.INSERT || record.getEventType()==EventType.UPDATE || record.getEventType()==EventType.DELETE) {
            for (EventColumn ec : keys) {
                if ("ID".equalsIgnoreCase(ec.getColumnName())) {
                    ...
                    String newValue = "polestar_" + oldValue;
