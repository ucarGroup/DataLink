 ...
 * @author lubiao
 */
public class CanalReaderMessageParser {

    private static final Logger logger = LoggerFactory.getLogger(CanalReaderMessageParser.class);
    ...
    public CanalReaderMessageParser(TaskReaderContext context, MysqlReaderParameter readerParameter) {
        ...
        for (RdbEventRecord record : records) {
            ...
            if (result && !record.getEventType().isDdl()) {
                throw new DatalinkException("ddl/dml can't be in one batch, it's may be a bug , pls submit issues.\r\n" +
                        CanalReaderRecordsDumper.buildOutputStr(records));
            }
        }
