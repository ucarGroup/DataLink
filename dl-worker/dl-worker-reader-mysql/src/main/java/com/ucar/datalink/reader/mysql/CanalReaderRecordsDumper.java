 ...
 * @author lubiao
 */
public class CanalReaderRecordsDumper {
    private static final Logger logger = LoggerFactory.getLogger(CanalReaderRecordsDumper.class);
    ...
    public static void dumpRecords(Long batchId, List<RdbEventRecord> records, String startPosition, String endPosition, int total, boolean isDumpDetail) {
        logger.info(SEP + "****************************************************" + SEP);
        logger.info(CanalReaderRecordsDumper.dumpMessageInfo(batchId, records, startPosition, endPosition, total));
        logger.info(SEP + "****************************************************" + SEP);
        if (isDumpDetail) {// 判断一下是否需要打印详细信息
            dumpEventRecords(records);
            logger.info(SEP + "****************************************************" + SEP);
        }
    }
    ...
    public static String dumpMessageInfo(Long batchId, List<RdbEventRecord> records, String startPosition, String endPosition, int total) {
        Date now = new Date();
        SimpleDateFormat format = new SimpleDateFormat(TIMESTAMP_FORMAT);
        int normal = records.size();
        return MessageFormat.format(context_format, batchId, total, normal, (total - normal),
