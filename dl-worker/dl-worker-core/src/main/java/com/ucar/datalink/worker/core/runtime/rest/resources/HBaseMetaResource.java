...
public class HBaseMetaResource {
    ...
    @POST
    @Path("/status")
    public String getStatus(HBaseParameterVO vo) {
        if (logger.isDebugEnabled()) {
            logger.debug("start getStatus method");
        }
        String check = check(vo);
        if (!StringUtils.isEmpty(check)) {
            return check;
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("execute getStatus method success");
            }
        }
        return "{}";
    }
    ...
    public String getColumns(HBaseParameterVO vo) {
        ...
        int fetchAmount = vo.getOnceFethAmount();
        if (tableName == null) {
            return "{}";
        }
        HBaseColumnsEvent event = new HBaseColumnsEvent(new FutureCallback(), hbaseParameter, zkParameter, tableName, fetchAmount);
        eventBus.post(event);
        try {
