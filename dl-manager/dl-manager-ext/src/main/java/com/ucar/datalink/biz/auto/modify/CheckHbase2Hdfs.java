...
public class CheckHbase2Hdfs implements ICheck {
    ...
    public void check(MediaSourceInfo srcInfo, MediaSourceInfo destInfo, JobConfigInfo info) throws Exception {
        ...
        //获取最新的Hive列信息
        List<ColumnMeta> columns = HBaseUtil.getColumns(srcInfo, SyncUtil.parseJobMediaName(info.getJob_media_name()));
        columns.forEach(c->{
        ...
        for(ModifyCheckColumnInfo.NameType nt : nameTypes) {
            if( map.containsKey(nt.name) ) {
                ...
                String hiveType = SyncModifyUtil.parseColumnToHiveType(mm, cMeta);
        ...
        SyncModifyUtil.modifyJsonColumn(info,srcModifyColumnInfo,destModifyColumnInfo);
        ...
        for(int i=0;i<addColumns.size();i++) {
            ...
            hBaseColumnInfo.setHiveType( SyncModifyUtil.parseColumnToHiveType(mm, addColumns.get(i)) );
        ...
        for(int i=0;i<modifyColumns.size();i++) {
            ...
            hBaseColumnInfo.setHiveType( SyncModifyUtil.parseColumnToHiveType(mm, modifyColumns.get(i)) );
        ...
        if(addColumn.length > 0) {
            SyncModifyUtil.sendModifyColumn(add);
        }
        if(modifyColumn.length > 0) {
            SyncModifyUtil.sendModifyColumn(modify);
        }
    }
