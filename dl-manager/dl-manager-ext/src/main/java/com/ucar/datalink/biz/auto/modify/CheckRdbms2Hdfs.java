...
public class CheckRdbms2Hdfs implements ICheck {
    ...
    public void check(MediaSourceInfo srcInfo, MediaSourceInfo destInfo, JobConfigInfo info) throws Exception {
        ...
        //获取最新的Hive列信息
        List<ColumnMeta> columns = SyncModifyUtil.getNewColumnInfo(srcInfo,info.getJob_media_name());
        ...
        for(int i=0;i<addColumns.size();i++) {
            ...
            columnInfo.setHiveType( SyncModifyUtil.parseColumnToHiveType(mm, addColumns.get(i)) );
        ...
        for(int i=0;i<modifyColumns.size();i++) {
            ...
            columnInfo.setHiveType( SyncModifyUtil.parseColumnToHiveType(mm, modifyColumns.get(i)) );
