...
import com.ucar.datalink.biz.module.SqlServerJobExtendProperty;
 ...
 */
public class GreenPlumJobConfigServiceImpl extends AbstractJobConfigService {


    private static final Logger LOGGER = LoggerFactory.getLogger(GreenPlumJobConfigServiceImpl.class);
    ...
    public String createReaderJson(MediaSourceInfo info, List<ColumnMeta> metas, JobExtendProperty property, String mediaName) {
        ...
        String json = "";
        try{
            String etlUrl = MessageFormat.format(DataxJobConfigConstant.GREENPLUM_URL, etlHost, port, schema);
            String url = MessageFormat.format(DataxJobConfigConstant.GREENPLUM_URL, ip, port, schema);
            String columns = buildColumnParm( metas );
            String reader = loadJobConfig(DataxJobConfigConstant.GREENPLUM_READER);
            ...
            LOGGER.error("greenplum createReaderJson error ",e);
        }
        return json;
    }
    ...
    public String createWriterJson(MediaSourceInfo srcInfo, MediaSourceInfo info, MediaMeta srcMediaMeta, JobExtendProperty property, String mediaName) {
        ...
        try{
            ...
            String url = MessageFormat.format(DataxJobConfigConstant.GREENPLUM_URL, ip, port, sehema);
            String columns = buildColumnParm( target.getColumn() );
            String reader = loadJobConfig(DataxJobConfigConstant.GREENPLUM_WRITER);
            json = replace(reader,url,username,password,columns);
            json = replaceSingleTable(json, parseMediaName(mediaName) );
        }catch (Exception e){
            LOGGER.error("greenplum createWriterJson error ",e);
        }
        return json;
    }
    ...
    public String reloadReader(String json,MediaSourceInfo info) {
        try {
            ...
            if(parameter.getReadConfig().getHosts()!=null && parameter.getReadConfig().getHosts().size()>0) {
                ...
                throw new RuntimeException("greenplum read ip is emtpy");
            }
            String etlHost = parameter.getReadConfig().getEtlHost();
            if(StringUtils.isBlank(etlHost)) {
                etlHost = ip;
