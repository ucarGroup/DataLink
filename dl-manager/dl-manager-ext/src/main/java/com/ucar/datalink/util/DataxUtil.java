...
import com.ucar.datalink.biz.module.DataxMachineInfo;
...
public class DataxUtil {
     ...
     * @return
     */
    public static RunningData getRunningData(String job_name) {
        try {
            String data = client.readData(DATAX_ADMIN_JOBS_RUNNING +"/"+ job_name, true);
            if(data != null) {
                return JSONObject.parseObject(data, RunningData.class);
            }
     ...
     * @return
     */
    public static DataxMachineInfo getDataxMachineInfo(String ip) {
        try {
            ...
            return JSONObject.parseObject(result,DataxMachineInfo.class);
        } catch(Exception e) {
            logger.error(e.getMessage(),e);
            return new DataxMachineInfo();
        }
    }
    ...
    public static String dynamicChoosenDataxMacheine() {
        ...
        Map<String,DataxMachineInfo> map = new HashedMap();
        String iteratorAddress = "";
        long lastIteratorMaxMemory = Long.MAX_VALUE;
        for(int i=0;i<addresses.size();i++) {
            DataxMachineInfo info = getDataxMachineInfo(addresses.get(i));
