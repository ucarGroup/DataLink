...
*/
        //DoubleCenterDataxService service = DataLinkFactory.getObject(DoubleCenterDataxService.class);
        //service.switchDataSourceForAllTimingJob();
        return "success";
    }
    ...
    public Map<String, Object> dbTypeChange(@ModelAttribute("name") String name, HttpServletRequest request) {
        ...
        list.addAll( VirtualDataSourceUtil.findMediaSourcesForAcrossLab(types) );
    ...
    public Map<String, Object> crossDataCenter(@ModelAttribute("name") String name, HttpServletRequest request) {
        ...
        List<MediaSourceInfo> mediaSourceList = VirtualDataSourceUtil.findMediaSourcesForAcrossLab(types);
        Map<String, Object> map = new HashMap<>();
        List<String> num = new ArrayList<>();
        List<String> val = new ArrayList<>();
    ...
    public List<String> namespaceContent(@ModelAttribute("id") String id, HttpServletRequest request) {
        ...
        MediaSourceInfo info = VirtualDataSourceUtil.getRealMediaSourceInfoById(mid);
        try {
            ...
            //处理分表
            TableModeUtil.doTableModel(result);
    ...
    public Object doParseJobContent(@ModelAttribute("srcID") String srcID, @ModelAttribute("destID") String destID, @ModelAttribute("mediaName") String mediaName) {
        ...
        MediaSourceInfo srcInfo = VirtualDataSourceUtil.getRealMediaSourceInfoById(Long.parseLong(srcID));
        MediaSourceInfo destInfo = VirtualDataSourceUtil.getRealMediaSourceInfoById(Long.parseLong(destID));
        try {
        ...
        public String doStart(HttpServletRequest request) {
            ...
            for (String jobId : jobIdArr){
                ...
                if(doubleCenterDataxService.isSwitchLabIng(info.getJob_src_media_source_id()) || doubleCenterDataxService.isSwitchLabIng(info.getJob_target_media_source_id())){
                    String msg = "当前job(" + info.getJob_name() + ")关联的数据源正在切机房中，job暂时不能启动，请稍后再试！";
                    logger.warn(msg);
                    return "当前job关联的数据源正在切机房中，job暂时不能启动，请稍后再试！";
                }
            }
...
//        } else {
//            SyncModifyUtil.checkModifyColumn(info);
//        }
                SyncModifyUtil.checkModifyColumn(info);
    ...
    public String doAdd(@ModelAttribute("view") JobConfigView view) {
        try {
            ...
            if(StringUtils.isNotBlank(jobContent) && jobContent.contains(SyncUtil.ESPECIALLY_CHAR_AND)) {
                jobContent = jobContent.replace(SyncUtil.ESPECIALLY_CHAR_AND,SyncUtil.ESPECIALLY_CHAR_AND_ORGINAL);
            ...
            if (view.getSrcType().toUpperCase().equals(MediaSourceType.HBASE.name())) {
                ...
                if (count >= 1) {
                    MediaSourceInfo info = VirtualDataSourceUtil.getRealMediaSourceInfoById(new Long(srcID));
                ...
                List<MediaSourceInfo> list = VirtualDataSourceUtil.fromRealMediaSourceToAllMediaSource(new Long(destID));
                JobConfigInfo info = new JobConfigInfo();
                info.setJob_name(view.getJob_name());
                info.setJob_content(JobConfigBuilder.decryptPassword(view.getJob_content()));
                info.setJob_media_name(view.getJob_media_name());
        ...
        try {
            ...
            for (int i = 0; i < src_names.length; i++) {
                ...
                if (Boolean.parseBoolean(timing_yn) && Boolean.parseBoolean(schedule_yn)) {
                    if(destInfo.getType()==MediaSourceType.ELASTICSEARCH) {
                        List<MediaSourceInfo> mediaSourceInfos = VirtualDataSourceUtil.fromRealMediaSourceToAllMediaSource(Long.parseLong(destID));
    ...
    public String toReloadJobContent(HttpServletRequest request) {
        try {
            ...
            JobConfigInfo jobConfigInfo = jobService.getJobConfigById(Long.parseLong(id));
            SyncModifyUtil.checkModifyColumnWithoutOpen(jobConfigInfo);
            return "success";
        } catch(Exception e) {
            logger.error(e.getMessage(),e);
            return e.getMessage();
    ...
    public String caclHbaseRegionCount(@ModelAttribute("id") String id, @ModelAttribute("tableName") String tableName) {
        MediaSourceInfo info = VirtualDataSourceUtil.getRealMediaSourceInfoById(Long.parseLong(id));
    ...
    public String getJVMSize() {
        String maxSize = EnvUtil.maxJVMMemory();
        String minSize = EnvUtil.minJVMMemory();
    ...
    @Transactional
    public void createAndSend(JobConfigInfo info) {
        try {
            SyncUtil.parseAndSetJobSrcTargetTable(info);
    ...
    private void createSchedule(JobConfigInfo info) {
        try {
            ...
            //JobScheduleInfo scheduleInfo = HttpCronUtil.assembleDefaultScheduleInfo(jobId);
            ...
            //HttpQuartzJob job = HttpCronUtil.assembleCronTaskWithMD5(info, scheduleInfo);
            //HttpCronUtil.schdule(job);
