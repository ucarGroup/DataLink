...
base=`dirname $0`/..
pidfile=$base/bin/manager.pid
if [ ! -f "$pidfile" ];then
	echo "manager is not running. exists"
	exit
fi

pid=`cat $pidfile`
if [ "$pid" == "" ] ; then
	pid=`get_pid "appName=datalink-manager"`
fi

echo -e "`hostname`: stopping manager $pid ... "
kill $pid

LOOPS=0
while (true);
do
	gpid=`get_pid "appName=datalink-manager" "$pid"`
    if [ "$gpid" == "" ] ; then
