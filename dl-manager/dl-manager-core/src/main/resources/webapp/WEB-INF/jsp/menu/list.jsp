...
<script type="text/javascript">
    ...
    msgAlarmListMyTable = $('#menuTable').DataTable({
        "bAutoWidth": true,
        "ajax": {
    ...
    function toAdd() {
        ...
        $("#add").load("${basePath}/menu/toAdd?random=" + Math.random());
        $("#add").show();
        $("#mainContentInner").hide();
    }
    function toEdit(id) {
        reset();//每次必须先reset，把已开界面资源清理掉
        $("#edit").load("${basePath}/menu/toEdit?id=" + id + "&random=" + Math.random());
        $("#edit").show();
        $("#mainContentInner").hide();
    }
    ...
    function doDelete(id) {
        if (confirm("确定要删除数据吗？")) {
            $.ajax({
                type: "post",
                url: "${basePath}/menu/doDelete?id=" + id,
                dataType: "json",
                async: false,
