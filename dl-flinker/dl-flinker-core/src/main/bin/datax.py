...
ENGINE_COMMAND = "java -server ${jvm} %s -classpath %s  ${params} com.ucar.datalink.flinker.core.Engine -mode ${mode} -jobid ${jobid} -executeId ${executeId} -timingJobId ${timingJobId} -job ${job} -jqeid ${jqeid}" % (
...
def getOptionParser():
    ...
    prodEnvOptionGroup.add_option("--executeId", metavar="<job execution unique id>", dest="executeId", action="store", default="-1",
                                  help="Set job execution unique id when running by Distribute/Local Mode.")
    prodEnvOptionGroup.add_option("--timingJobId", metavar="<job timer unique id>", dest="timingJobId", action="store", default="-1",
                                  help="timer job id")
    ...
    prodEnvOptionGroup.add_option("--jqeid", metavar="<job queue id>", dest="jqeid", action="store",
                                  default="-1", help="Set job execution id when job is started in job queue.")
...
def buildStartCommand(options, args):
    ...
    if options.jvmParameters:
        tempJVMCommand = options.jvmParameters
	...
	if options.executeId:
		commandMap["executeId"] = options.executeId
		
    if options.timingJobId:
        commandMap["timingJobId"] = options.timingJobId
    
    if options.jqeid:
    	commandMap["jqeid"] =options.jqeid
