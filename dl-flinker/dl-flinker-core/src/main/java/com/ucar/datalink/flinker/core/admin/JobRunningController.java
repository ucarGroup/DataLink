...
//				logger.info("handleSessionEstablishmentError -> "+throwable);
//			}
		};
	...
	public void start() {
		String workerPath = ZookeeperPathUtils.getWorkerPath(HostUtils.IP);
		String flowPath = ZookeeperPathUtils.getMonitorPath(HostUtils.IP);
		zkClient.createEphemeral(workerPath);
		...
		Thread t = new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					zkClient.delete(ZookeeperPathUtils.getJobRunningPath(command.getJobName()));
	...
	private void stopJob(final Command command) {
		try {
			String data = null;
			byte[] bytes = zkClient.readData(ZookeeperPathUtils.getJobRunningPath(command.getJobName()), true);
			if (bytes != null) {
	...
	private void updateRunningData(final RunningData runningData) {
		zkClient.updateDataSerialized(ZookeeperPathUtils.getJobRunningPath(runningData.getJobName()), new DataUpdater<byte[]>() {
