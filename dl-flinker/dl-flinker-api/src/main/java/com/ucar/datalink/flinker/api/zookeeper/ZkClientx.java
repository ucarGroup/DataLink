...
public class ZkClientx extends ZkClient {
    ...
    // 对于zkclient进行一次缓存，避免一个jvm内部使用多个zk connection,data-link专用
    private static Map<String, ZkClientx> clients_datalink = MigrateMap.makeComputingMap(new Function<String, ZkClientx>() {

                                                      public ZkClientx apply(String servers) {
                                                          return new ZkClientx(new ZooKeeperx(servers),Integer.MAX_VALUE,new SerializableSerializer());
                                                      }
                                                  });
    ...
    public static ZkClientx getZkClientForDatalink(String servers) {
        return clients_datalink.get(servers);
    }
