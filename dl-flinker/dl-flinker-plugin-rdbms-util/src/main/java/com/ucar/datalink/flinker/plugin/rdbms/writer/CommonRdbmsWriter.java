package com.ucar.datalink.flinker.plugin.rdbms.writer;
...
import com.ucar.datalink.flinker.plugin.rdbms.writer.util.OriginalConfPretreatmentUtil;
import com.ucar.datalink.flinker.plugin.rdbms.writer.util.WriterUtil;
...
public class CommonRdbmsWriter {
    ...
    public static class Task {
        ...
        /**
         * 处理关键字冲突问题，即表中的字段名和数据库的关键字冲突了
         * 不同的数据库处理冲突的方式不同，mysql是在字段前后加反引号，如 `DESC`
         * sqlserver是在字段前后加 []
         * 目前只处理mysql和sqlserver
         * @param columns
         */
        public List<String> dealKeyConflict(List<String> columns) {
        ...
        protected PreparedStatement fillPreparedStatementColumnType(PreparedStatement preparedStatement, int columnIndex, int columnSqltype, Column column) throws SQLException {
            ...
            switch (columnSqltype) {
                ...
                case Types.DATE:
                    if ((DataBaseType.MySql==dataBaseType) && (this.resultSetMetaData.getRight().get(columnIndex).equalsIgnoreCase("year")) ) {
                ...
                case Types.BIT:
                    int v = column.asLong().intValue();
                    preparedStatement.setInt(columnIndex + 1, v);
                    break;
                default:
                    throw DataXException
                            .asDataXException(
