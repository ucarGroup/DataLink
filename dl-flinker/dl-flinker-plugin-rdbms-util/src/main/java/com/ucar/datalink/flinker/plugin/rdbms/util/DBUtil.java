...
import com.ucar.datalink.flinker.api.util.RetryUtil;
...
//                    throw new Exception(DBUtilErrorCode.JDBC_NULL.toString());
    ...
    public static String chooseJdbcUrlWithoutRetry(final DataBaseType dataBaseType,
            ...
            if (StringUtils.isNotBlank(url)) {
                ...
                if (null != preSql && !preSql.isEmpty()) {
                    ...
                    try {
                        ...
                        LOG.error(String.format("execute sql url[%s] username[%s]",url,username));
                        ErrorRecord.addError(String.format("execute sql url[%s] username[%s]",url,username));
                        throw DataXException.asDataXException(
                                DBUtilErrorCode.CONN_DB_ERROR,
                                String.format("数据库连接失败. 因为根据您配置的连接信息,无法从:%s 中找到可连接的jdbcUrl. 请检查您的配置并作出修改.",
                                        StringUtils.join(jdbcUrls, ",")), e);
        ...
        ErrorRecord.addError(String.format("数据库连接失败. 因为根据您配置的连接信息,无法从:%s 中找到可连接的jdbcUrl. 请检查您的配置并作出修改.",
                StringUtils.join(jdbcUrls, ",")));
        throw DataXException.asDataXException(
                DBUtilErrorCode.CONN_DB_ERROR,
                String.format("数据库连接失败. 因为根据您配置的连接信息,无法从:%s 中找到可连接的jdbcUrl. 请检查您的配置并作出修改.",
    ...
    public static boolean hasInsertPrivilege(DataBaseType dataBaseType, String jdbcURL, String userName, String password, List<String> tableList) {
        ...
        try {
        	...
        	ErrorRecord.addError("Check the database has the Insert Privilege failed, errorMessage:[{}]"+e.getMessage());
    ...
    private static synchronized Connection connect(DataBaseType dataBaseType,
        ...
        try {
            ...
            if(dataBaseType == DataBaseType.MySql || DataBaseType.Tddl  == dataBaseType || DataBaseType.DRDS == dataBaseType){
                return Mysql8Utils.getMysqlConnection(url,prop);
            }
            return DriverManager.getConnection(url, prop);
        } catch (Exception e) {
            LOG.error(String.format("connnection rdb dataBaseType[%s] url[%s] prop[%s]",dataBaseType.toString(),url,prop.toString()));
    ...
    public static boolean asyncResultSetNext(final ResultSet resultSet) {
        LOG.info("SingleTableSplitUtil#checkSplitPk() -> DBUtil.asyncResultSetNext()");
