...
/**
 * Created by yang.wang09 on 2019-05-08 15:34.
 */
public class HANAJobConfigServiceImpl extends OracleJobConfigServiceImpl {

        private static final Logger LOGGER = LoggerFactory.getLogger(HANAJobConfigServiceImpl.class);
    ...
    public String createReaderJson(MediaSourceInfo info, List<ColumnMeta> metas, JobExtendProperty property, String mediaName) {
        ...
        String json = "";
        try{
            String etlUrl = MessageFormat.format(FlinkerJobConfigConstant.HANA_URL, etlHost, port, schema);
            String url = MessageFormat.format(FlinkerJobConfigConstant.HANA_URL, ip, port, schema);
            String columns = buildColumnParm( metas );
            String reader = loadJobConfig(FlinkerJobConfigConstant.HANA_READER);
