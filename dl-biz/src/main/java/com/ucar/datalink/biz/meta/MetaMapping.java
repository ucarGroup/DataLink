...
*/
        ...
        || meta.getDbType()==MediaSourceType.SQLSERVER || meta.getDbType()==MediaSourceType.POSTGRESQL || meta.getDbType()==MediaSourceType.HANA) {
            //将关系型数据库转换成HDFS
            return transform(meta, new ColumnTransform() {
                public ColumnMeta execute(ColumnMeta c) {
    ...
    public static MediaMeta transformToKUDU(MediaMeta meta) {
        if ( meta.getDbType()==MediaSourceType.MYSQL || meta.getDbType()==MediaSourceType.ORACLE || meta.getDbType()==MediaSourceType.SQLSERVER || meta.getDbType()==MediaSourceType.POSTGRESQL ) {
            return transform(meta, new ColumnTransform() {
                public ColumnMeta execute(ColumnMeta c) {
                    return rdbmsMapping.toKudu(c);
                }
